...
st.title("🏭 Inteligencia de Procesos Metalúrgicos")
...
if archivo:
    ...
    tab_vista, tab_limpieza, tab_modelo, tab_analisis, tab_simulador = st.tabs([
        "👁️ 1. Vista Previa", "🧹 2. Limpieza", "🛠️ 3. Entrenamiento", "📊 4. Explorador X-Y", "🎯 5. Panel de Control"
    ])

    # --- TAB 1: VISTA PREVIA (EL QUE FALTABA) ---
    with tab_vista:
        ...
        st.write(f"**Total de muestras:** {df.shape[0]} | **Total de variables:** {df.shape[1]}")

    # --- TAB 2: LIMPIEZA ---
    with tab_limpieza:
        st.subheader("🧹 Gestión Manual de Outliers")
        ...
        outliers = df[(df[col_out] < inf) | (df[col_out] > sup)].copy()
        ...
        if not outliers.empty:
            ...
            st.success("No se detectaron outliers automáticos.")
            df_limpio = df.copy()
    ...
    with tab_modelo:
        ...
        if st.button("🚀 Entrenar Modelo de Planta", use_container_width=True):
    ...
    # --- TAB 5: PANEL DE CONTROL (WHAT-IF) ---
    with tab_simulador:
        if 'mod' in st.session_state:
            st.subheader("🎯 Simulador y Análisis de Sensibilidad")
            col_sl, col_ga = st.columns([1, 2])

            with col_sl:
                st.markdown("### 🎛️ Control de Variables")
                inputs = {}
            ...
            with col_ga:
                ...
                fig_sens = px.bar(x=sens_df.values, y=sens_df.index, orientation='h',
                                  title="¿Qué variable afecta más la recuperación hoy?",
                                  color=sens_df.values, color_continuous_scale="RdYlGn")
